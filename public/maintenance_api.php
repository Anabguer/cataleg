    ...
    if ($action === 'save') {
        ...
        try {
            ...
            maintenance_save(db(), $module, $year, $originalForSave, [
                ...
                'org_dependency_id' => (string) ($in['org_dependency_id'] ?? ''),
                'contribution_epigraph_id' => (string) ($in['contribution_epigraph_id'] ?? ''),
                'contribution_group_id' => (string) ($in['contribution_group_id'] ?? ''),
                'civil_service_scale_id' => (string) ($in['civil_service_scale_id'] ?? ''),
                'civil_service_subscale_id' => (string) ($in['civil_service_subscale_id'] ?? ''),
                'civil_service_class_id' => (string) ($in['civil_service_class_id'] ?? ''),
                'civil_service_category_id' => (string) ($in['civil_service_category_id'] ?? ''),
                'job_type_id' => (string) ($in['job_type_id'] ?? ''),
                'classification_group_new' => (string) ($in['classification_group_new'] ?? ''),
                'organic_level' => (string) ($in['organic_level'] ?? ''),
                ...
                'has_night_schedule' => $in['has_night_schedule'] ?? 0,
                'has_holiday_schedule' => $in['has_holiday_schedule'] ?? 0,
                'has_shift_schedule' => $in['has_shift_schedule'] ?? 0,
                'has_special_dedication' => $in['has_special_dedication'] ?? 0,
                'special_dedication_type' => (string) ($in['special_dedication_type'] ?? ''),
                'availability_id' => (string) ($in['availability_id'] ?? ''),
                'mission' => (string) ($in['mission'] ?? ''),
                'generic_functions' => (string) ($in['generic_functions'] ?? ''),
                'specific_functions' => (string) ($in['specific_functions'] ?? ''),
                'qualification_requirements' => (string) ($in['qualification_requirements'] ?? ''),
                'other_requirements' => (string) ($in['other_requirements'] ?? ''),
                'training_requirements' => (string) ($in['training_requirements'] ?? ''),
                'experience_requirements' => (string) ($in['experience_requirements'] ?? ''),
                'other_merits' => (string) ($in['other_merits'] ?? ''),
                'provision_method_id' => (string) ($in['provision_method_id'] ?? ''),
                'effort' => (string) ($in['effort'] ?? ''),
                'hardship' => (string) ($in['hardship'] ?? ''),
                'danger' => (string) ($in['danger'] ?? ''),
                'incompatibilities' => (string) ($in['incompatibilities'] ?? ''),
                'provincial_notes' => (string) ($in['provincial_notes'] ?? ''),
                'work_center_id' => (string) ($in['work_center_id'] ?? ''),
                'creation_reason' => (string) ($in['creation_reason'] ?? ''),
                'deletion_reason' => (string) ($in['deletion_reason'] ?? ''),
                'job_evaluation' => (string) ($in['job_evaluation'] ?? ''),
                'assigned_person_ids' => $in['assigned_person_ids'] ?? [],
            ]);
